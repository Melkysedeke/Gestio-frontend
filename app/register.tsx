...
export default function RegisterScreen() {
  ...
  const handleAddPhoto = async () => {
    const result = await ImagePicker.launchImageLibraryAsync({
      ...
      allowsEditing: true,
      aspect: [1, 1],
      quality: 0.7, // Pode usar qualidade boa, pois vai pro Storage!
    });

    if (!result.canceled) {
      setSelectedImage(result.assets[0].uri); // Pegamos a URI normal, sem Base64!
    }
  };
...
const styles = StyleSheet.create({
  ...
  textSection: { marginBottom: 24, alignItems: 'center'},
  title: { fontSize: 32, fontWeight: '800', color: '#0f172a', marginBottom: 8, lineHeight: 38, alignItems: 'center' },
  subtitle: { fontSize: 16, color: '#64748b', alignItems: 'center' },
