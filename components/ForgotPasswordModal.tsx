...
export default function ForgotPasswordModal({ visible, onClose, initialEmail = '' }: ForgotPasswordModalProps) {
  ...
  // 🚀 A Solução Definitiva: Animando o Padding Fisicamente
  ...
  useEffect(() => {
    ...
    const showSub = Keyboard.addListener(showEvent, (e) => {
      RNAnimated.timing(keyboardPadding, {
        ...
        duration: 250,
        useNativeDriver: false, // 👈 FALSE obriga o Android a redesenhar a tela sem deixar buracos
      }).start();
    });

    const hideSub = Keyboard.addListener(hideEvent, () => {
      RNAnimated.timing(keyboardPadding, {
        toValue: 0, // 👈 Volta exato para o zero, colando no chão
        duration: 250,
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={handleClose}>
      ...
      <View style={styles.modalOverlay}>
        ...
        {/* Fundo escuro que cobre a tela toda. Não se move. */}
        <TouchableWithoutFeedback onPress={handleClose}>
          <View style={StyleSheet.absoluteFill} />
        </TouchableWithoutFeedback>

        {/* 🚀 Wrapper que empurra o conteúdo para cima usando o Padding Animado */}
